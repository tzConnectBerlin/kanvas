...
const TreeView: FC<TreeViewProps> = ({
  ...
  const select = (node: any) => {
    ...
    if (selectedFilters.indexOf(node.id) > -1) {
      ...
      setSelectedFilters([
        ...selectedFilters.filter((id) => selectedNodes.indexOf(id) === -1),
        ...selectedNodes,
      ])
      setHighlightedParents([
        ...
        >
          <StyledLi open={props.open} key={`${node.name}-${node.id}`}>
            <Stack direction="row" sx={{ alignItems: 'center', width: '100%' }}>
              ...
              <Stack
                direction="row"
                onClick={() => handleListItemClick(`${node.name}-${node.id}`)}
                sx={{ width: '100%' }}
              >
                <Typography
                  size="h5"
                  weight="Light"
                  color={
                    highlightedParentsState.indexOf(node.id) !== -1 &&
                    selectedFilters.indexOf(node.id) === -1
                      ? 'contrastText'
                      : ''
                  }
                >
                  {node.name} - {countElInArr(node.id, highlightedParentsState)}
                </Typography>
