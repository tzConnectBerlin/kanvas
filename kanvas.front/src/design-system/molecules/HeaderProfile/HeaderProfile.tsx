    ...
    return (
        <>
            ...
            <DesktopWrapperStack direction='row' spacing={9}>
                <StyledPictureStack direction='column' spacing={3} sx={{maxWidth: 200, justifyContent: 'center'}}>
                ...
                <Stack direction='column' sx={{width: '100%', justifyContent: 'center'}}>
                    <Stack direction='row' spacing={2} sx={{ alignItems: 'center', width: '100%'}}>
                        <Typography size="h2" weight='SemiBold' noWrap={true} align='left' sx={{marginRight: '2rem'}} display='block'> { props.loading ? <Skeleton width='15rem'/> : props.user?.name } </Typography>

                        <FlexSpacer minWidth={0} />

                        {
                            // Here goes validation if user profile is logged in user
                            localStorage.getItem('Kanvas - address') === props.user?.address ?
                    ...
                    <AddressStack direction='row' spacing={2} sx={{maxWidth: 400 ,alignItems: 'center'}}>
                        <Typography size="h5" weight='Light' color='#C4C4C4' noWrap={true} align='center' display='initial !important' width='16rem'> { props.loading ? <Skeleton width='5rem'/> : props.user?.address } </Typography>
