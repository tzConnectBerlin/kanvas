...
export const StoreFilters: FC<StoreFiltersProps> = ({ children, ...props }) => {
  ...
  const handleListItemClick = (concernedRef: string) => {
    if (activeRef.indexOf(concernedRef) !== -1) {
      setActiveRef(activeRef.filter((ref: string) => ref !== concernedRef))
    } else {
    ...
    <StyledUl openFilters={props.openFilters}>
      ...
      >
        ...
        <Typography
          ...
          size="subtitle2"
          weight={props.selectedFilters.length > 0 ? 'Medium' : 'Light'}
          color={props.selectedFilters.length > 0 ? 'contrastText' : '#C4C4C4'}
