-- Migration: nft_editions_locked_v4
-- Created at: 2022-05-31 15:11:34
-- ====  UP  ====

BEGIN;

ALTER FUNCTION nft_editions_locked RENAME TO __nft_editions_locked_v3;
CREATE FUNCTION nft_editions_locked(INT)
  RETURNS TABLE(reserved BIGINT, owned BIGINT)
AS $$
  SELECT
    count(cart_nfts) FILTER (WHERE payment.id IS NULL)
    + count(order_nfts) FILTER (WHERE payment.id IS NOT NULL) AS reserved,
...
ALTER FUNCTION __nft_editions_locked_v3 RENAME TO nft_editions_locked;
