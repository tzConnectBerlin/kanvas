      ...
      case NotificationEnum.FOLLOWING:
        return (
          <Typography
            ...
            onClick={() =>
              navigateTo(`/profile/${notification.concernedUser.userName}`)
            }
          >
            <StyledMediumSpan>
              {' '}
              {notification.concernedUser.firstName +
                ' ' +
                notification.concernedUser.lastName +
                ' '}
            </StyledMediumSpan>
      ...
      case NotificationEnum.NFT_CREATION:
        return (
          <Stack direction="row" spacing={0.7}>
            ...
            >
              {' '}
              {'"' + notification.concernedNft?.title + '"'}{' '}
            </Typography>
            <Typography size="h4" weight="Light" sx={{ cursor: 'pointer' }}>
              {' '}
          ...
          <Stack direction="row" spacing={0.7}>
            ...
            >
              ...
              <StyledLightSpan>
                {' '}
                got sold for{' '}
                {notification.saleInfo!.price +
                  notification.saleInfo!.currency}{' '}
                to{' '}
              </StyledLightSpan>
              <StyledMediumSpan>
                {' '}
              ...
              <StyledLightSpan>
                {' '}
                {'you received ' +
                  notification.saleInfo!.sellerPrice +
                  notification.saleInfo!.currency +
                  '.'}{' '}
              </StyledLightSpan>
            </Typography>
        ...
        <Stack direction="column">
          {props.notifications.map((notification: INotification) => (
            ...
            >
              <Avatar
                src={
                  notification.type === NotificationEnum.FOLLOWING
                    ? notification.concernedUser.profilePicture
                    : notification.concernedNft?.url
                }
                height={75}
                width={75}
                borderRadius={getAvatarRadius(notification.type)}
              />
              <Stack direction="column" sx={{ justifyContent: 'center' }}>
                ...
                >
                  {' '}
                  {getTimeInfo(notification.date)}{' '}
                </Typography>
              </Stack>
              <FlexSpacer />

              {notification.read ? undefined : <UnreadIndicator />}
            </StyledNotificationStack>
          ))}
        </Stack>
      </StyledStack>
